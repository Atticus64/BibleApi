import { Version, getFolder } from "$/scraping/scrape.ts";
...
export async function searchVersion(c: Context, version: Version) {
	...
	if (!take && !page) {
		...
		data = await search({ version, query: q, take: Number(take), testament: test });
	} else if (!take) {
		data = await search({ version, query: q, page: Number(page), testament: test });
		return c.json(data);
	} else {
		data = await search({ version, query: q, take: Number(take), page: Number(page), testament: test });
	}
...
export async function search({ version, query, take = 10, page = 1, testament = 'both' }: searchProps) {
	...
	if (testament === 'both') {
		info.forEach(({ book, chapter, verse, passage, study, id }) => {
			...
			if (verse.toLowerCase().includes(query.toLowerCase())) {
				...
				const url = `https://bible-study.vercel.app/chapter/${versionFolder}/${book.toLowerCase()}/${chapter}`
		...
		booksFound = books.filter(b => b.name.toLowerCase().includes(query.toLowerCase()))

	}

	if (testament === 'old') {
		info.filter(v => v.testament === 'old').forEach(({ book, chapter, verse, passage, study, id }) => {
		...
		booksFound = books.filter(b => b.testament === 'Antiguo Testamento').filter(b => b.name.toLowerCase().includes(query.toLowerCase()))

	} else if (testament === 'new') {
		info.filter(v => v.testament === 'new').forEach(({ book, chapter, verse, passage, study, id }) => {
		...
		booksFound = books.filter(b => b.testament === 'Nuevo Testamento').filter(b => b.name.toLowerCase().includes(query.toLowerCase()))
